         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {images.map((image, index) => (
                <div key={image.id} className="group relative bg-gray-800 rounded-lg overflow-hidden shadow-lg">
                    <img src={image.src} alt={`Generated Ad ${index + 1}`} className="w-full h-auto object-cover aspect-square"/>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            for (let i = 0; i < imagesToGenerate.length; i++) {
                setProgressMessage(`Generating Ad ${i + 1} of ${imagesToGenerate.length}...`);
                const prompt = imagesToGenerate[i];
                const imageUrl = await geminiService.generateAdImage(modelImage.file, productImage.file, prompt, logoImage?.file);
                newImages.push({ id: `img-${i}`, src: imageUrl, prompt, refinementText: '' });
